...
@section('content')
    ...
    <form action="{{route('deliversReports.index')}}">
    <div class="row mb-3">
        <div class="col-md-2 ">
        ...
        <div class="col-md-2">
            ...
            <select id="delivery_id" class="form-control showBonds" name="delivery_id">
                    ...
                    @foreach($deliveries as $delivery)
                        <option @isset($request['delivery_id']) @if($request['delivery_id']==$delivery->id) selected @endif   @endisset value="{{$delivery->id}}">{{$delivery->name}}</option>
                    @endforeach
                </select>
        ...
        <div class="col-md-2">
            ...
            <select id="order_status" class="form-control showBonds" name="status">
                <option selected disabled>اختر</option>
                <option @isset($request['status']) @if($request['status']=='new') selected @endif   @endisset value="new">جديد</option>
                <option @isset($request['status']) @if($request['status']=='converted_to_delivery') selected @endif   @endisset value="converted_to_delivery">محول الي مندوب</option>
                <option @isset($request['status']) @if($request['status']=='total_delivery_to_customer') selected @endif   @endisset value="total_delivery_to_customer">مسلم كليا</option>
...
@section('js')
    ...
    @include('Admin.layouts.inc.ajax',['url'=>'deliversReports'])
    ...
    <script>
        ...
        $(document).on('change', '.showBonds', function () {
            ...
            var url = "{{route('deliversReports.index')}}";
            url = url + "?-=-";
            if (fromDate != null) {
            ...
            if (delivery_id != null) {
                url = url + "&&delivery_id=" + delivery_id;
            }
            if (status != null) {
