...
@section('content')
    ...
    <div class="card">
        <div class="card-header ">
         ...
         <td> <a href="{{route('repository.edit',$row->id)}}"> تعديل </a>   
                       
          <form action="{{ route('repository.destroy', $row->id ) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                <button type="submit" class=" btn fa fa-trash text-danger" onclick="return confirm('Are you sure to delete this user?')"> ح1ف</button>
                                </form>
                                </td>
         </tr>
         @endforeach
