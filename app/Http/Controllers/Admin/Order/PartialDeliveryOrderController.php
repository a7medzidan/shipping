...
class PartialDeliveryOrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $admins = Order::query()->with(['province', 'trader', 'delivery'])->where('status', 'partial_delivery_to_customer')->orderBy('updated_at', 'desc');
            ...
            $this->add_log_activity(null, auth('admin')->user(), "تم عرض  الطلبات  المسلمة جزئيا للعميل");

        }
        return view('Admin.CRUDS.Orders.partialDeliveryOrders.index');
    }
